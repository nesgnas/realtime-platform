defmodule RelayWeb.FallbackController do
  ...
  def call(conn, {:error, %Ecto.Changeset{} = changeset}), do: conn |> put_status(:unprocessable_entity) |> json(%{error: %{code: "validation_failed", details: Ecto.Changeset.traverse_errors(changeset, fn {msg, opts} -> Enum.reduce(opts, msg, fn {k, v}, s -> String.replace(s, "%{#{k}}", to_string(v)) end) end)}})
