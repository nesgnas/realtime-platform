...
export function useWebRTC(channel: Channel | null, video: boolean, onEnded: () => void) {
  ...
  const start = useCallback(async (offer = true) => { const stream = await navigator.mediaDevices.getUserMedia({ audio: true, video }); local.current = stream; setLocalStream(stream); const pc = ensurePeer(); stream.getTracks().forEach(track => pc.addTrack(track, stream)); if (offer) { const sdp = await pc.createOffer(); await pc.setLocalDescription(sdp); send({ type: 'offer', sdp }) } }, [ensurePeer, send, video])
  useEffect(() => {
    if (!channel) return
    const handle = async (raw: unknown) => { const signal = raw as Signal; if (signal.type === 'hangup') return onEnded(); const pc = ensurePeer(); if (signal.type === 'ice') await pc.addIceCandidate(signal.candidate); else { await pc.setRemoteDescription(signal.sdp); if (signal.type === 'offer') { if (!local.current) await start(false); const answer = await pc.createAnswer(); await pc.setLocalDescription(answer); send({ type: 'answer', sdp: answer }) } } }
    const bindingRef = channel.on('signal', handle); return () => { channel.off('signal', bindingRef) }
  }, [channel, ensurePeer, onEnded, send, start])
