...
export function useWebRTC(channel: Channel | null, video: boolean, initiator: boolean, onEnded: () => void) {
  ...
  const ensurePeer = useCallback(() => {
    ...
    const pc = new RTCPeerConnection({ iceServers }); pc.onicecandidate = e => e.candidate && send({ type: 'ice', candidate: e.candidate.toJSON() }); pc.ontrack = e => setRemote(e.streams[0]); pc.onconnectionstatechange = () => setConnectionState(pc.connectionState); peer.current = pc; return pc
  }, [send])
