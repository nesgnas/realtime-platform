...
export function useWebRTC(channel: Channel | null, video: boolean, initiator: boolean, onEnded: () => void) {
  ...
  useEffect(() => {
    if (!channel) return
    const handle = async (raw: unknown) => { const signal = raw as Signal; if (signal.type === 'hangup') return onEnded(); const pc = ensurePeer(); if (signal.type === 'ice') { if (pc.remoteDescription) await pc.addIceCandidate(signal.candidate); else pendingCandidates.current.push(signal.candidate) } else { if (signal.type === 'offer' && !local.current) await start(); await pc.setRemoteDescription(signal.sdp); for (const candidate of pendingCandidates.current.splice(0)) await pc.addIceCandidate(candidate); if (signal.type === 'offer') { const answer = await pc.createAnswer(); await pc.setLocalDescription(answer); send({ type: 'answer', sdp: answer }) } } }
