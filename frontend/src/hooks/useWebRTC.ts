...
export function useWebRTC(channel: Channel | null, video: boolean, initiator: boolean, onEnded: () => void) {
  ...
  const peerPresent = useRef(false), pendingCandidates = useRef<RTCIceCandidateInit[]>([]), pendingOffer = useRef<RTCSessionDescriptionInit | null>(null)
  ...
  const [connectionState, setConnectionState] = useState<RTCPeerConnectionState>('new'), [error, setError] = useState('')
  ...
  const ensurePeer = useCallback(() => {
    ...
    const pc = new RTCPeerConnection({ iceServers }); pc.onicecandidate = e => e.candidate && send({ type: 'ice', candidate: e.candidate.toJSON() }); pc.ontrack = e => setRemote(current => { const stream = current || new MediaStream(); if (!stream.getTracks().some(track => track.id === e.track.id)) stream.addTrack(e.track); return stream }); pc.onconnectionstatechange = () => setConnectionState(pc.connectionState); peer.current = pc; return pc
  }, [send])
  ...
  const start = useCallback(async () => { try { setError(''); const stream = await navigator.mediaDevices.getUserMedia({ audio: true, video }); local.current = stream; setLocalStream(stream); const pc = ensurePeer(); stream.getTracks().forEach(track => pc.addTrack(track, stream)); if (initiator && peerPresent.current) await makeOffer(); else if (pendingOffer.current) { await pc.setRemoteDescription(pendingOffer.current); pendingOffer.current = null; for (const candidate of pendingCandidates.current.splice(0)) await pc.addIceCandidate(candidate); const answer = await pc.createAnswer(); await pc.setLocalDescription(answer); send({ type: 'answer', sdp: answer }) } } catch (reason) { setError(reason instanceof Error ? reason.message : 'Could not access media devices'); throw reason } }, [ensurePeer, initiator, makeOffer, send, video])
  useEffect(() => {
    if (!channel) return
    const handle = async (raw: unknown) => { try { const signal = raw as Signal; if (signal.type === 'hangup') return onEnded(); const pc = ensurePeer(); if (signal.type === 'ice') { if (pc.remoteDescription) await pc.addIceCandidate(signal.candidate); else pendingCandidates.current.push(signal.candidate) } else if (signal.type === 'offer' && !local.current) { pendingOffer.current = signal.sdp } else { await pc.setRemoteDescription(signal.sdp); for (const candidate of pendingCandidates.current.splice(0)) await pc.addIceCandidate(candidate); if (signal.type === 'offer') { const answer = await pc.createAnswer(); await pc.setLocalDescription(answer); send({ type: 'answer', sdp: answer }) } } } catch (reason) { setError(reason instanceof Error ? reason.message : 'Call negotiation failed') } }
    ...
    return () => { channel.off('signal', signalRef); channel.off('participant:joined', participantRef); peerPresent.current = false; pendingCandidates.current = []; pendingOffer.current = null }
  ...
  return { localStream, remote, muted, cameraOff, sharing, connectionState, error, start, stop, toggleMute, toggleCamera, shareScreen }
}
